...
def build_pipeline():
    ...
    enriched = enrich_chunks(all_chunks, methods=list(DEFAULT_ENRICHMENT_METHODS))
    if enriched:
...
def run_query(query: str, search: HybridSearch, reranker: CrossEncoderReranker) -> tuple[str, list[str]]:
    ...
    contexts = [r.text for r in reranked] if reranked else [r.text for r in results[:DEFAULT_CONTEXT_TOP_K]]

    # TODO (nhóm): Replace with LLM generation for better scores
    # from openai import OpenAI
    # client = OpenAI()
    # context_str = "\n\n".join(contexts)
    # resp = client.chat.completions.create(model=GENERATION_MODEL, messages=[
...
def evaluate_pipeline(search: HybridSearch, reranker: CrossEncoderReranker):
    ...
    for m in ["faithfulness", "answer_relevancy", "context_precision", "context_recall"]:
        s = results.get(m, 0)
        print(f"  {'✓' if s >= PASS_THRESHOLD else '✗'} {m}: {s:.4f}")
